...
server <- function(input, output, session) {
  ...
  returnTable1 <- reactive({
    ...
    vr1<-paste(rawData[(rawData$V1==1),]$V2)
    vr2<-paste(rawData[(rawData$V1==2),]$V2)
    ...
    adsl1 <- adsl %>% 
      filter(TRT01P==vr1 & SAFFL=='Y')
    
    adsl1$grp <- 'grp1'
  
    adsl2 <- adsl %>% 
      filter(TRT01P==vr2 & SAFFL=='Y')
    ...
    adae1 <- adae %>%
      filter(TRT01P==vr1 & SAFFL=='Y' & TRTEMFL=='Y')
    
    adae1$grp <- 'grp1'
    
    adae2 <- adae %>%
      filter(TRT01P==vr2 & SAFFL=='Y' & TRTEMFL=='Y')
    ...
    tab_html <- df %>% 
      ...
      tab_source_note(
        source_note = paste('Program Source: app2.r            Executed: (Draft)',  the_date)
      ) %>%
      
      cols_label(
        term = html("Preferred Term"),
        grp1_c = html(paste(vr1, " <br> (N=", bign[1], ")")),
        grp2_c = html(paste(vr2, " <br> (N=", bign[2], ")")),
